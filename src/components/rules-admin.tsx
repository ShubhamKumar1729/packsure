...
const kindLabels: Record<RuleKind, string> = {
  ...
  any_field_presence: 'Any-of-fields presence',
...
export function RulesAdmin() {
  ...
  const [acknowledgeVerified, setAcknowledgeVerified] = useState(false)

  const installBaseline = async (enable: boolean) => {
    if (enable && !acknowledgeVerified) {
      setError('Tick the verification acknowledgement before enabling the legal baseline.')
      return
    }
    ...
    setBaselineMessage('')
    try {
      const response = await fetch('/api/rules/legal-metrology', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ enable, acknowledgeVerified }),
      })
      const data = (await response.json()) as { created?: RuleRecord[]; skipped?: string[]; note?: string; error?: string }
      if (!response.ok) throw new Error(data.error || 'The legal baseline could not be installed.')
      setBaselineMessage(data.note || 'Baseline installed.')
      await loadRules()
    } catch (installError) {
      setError(installError instanceof Error ? installError.message : 'The legal baseline could not be installed.')
    } finally {
